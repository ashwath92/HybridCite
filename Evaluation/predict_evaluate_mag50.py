...
# LOAD MODELS
loadmodstart = time()
id2word_dictionary = corpora.Dictionary.load('/home/ashwath/Programs/MAGCS/LDA/mag50.dict')
...
malletindex = similarities.MatrixSimilarity.load('/home/ashwath/Programs/MAGCS/LDA/simIndexMag50.index')
with open('/home/ashwath/Programs/MAGCS/LDA/docid_to_magid_training_mag50.pickle', 'rb') as pick:
    docid_to_magid = pickle.load(pick)

hd2vmodel = HyperDoc2Vec.load('/home/ashwath/Programs/MAGCS/MAG-hyperdoc2vec/models/magcsenglish_window20_mincitations50.model')
...
def solr_recommend(context):
    ...
    num_rows = 500
    sleep(0.2)
    pred_docs_dict = search_solr_parse_json(context, 'mag_en_cs_50', 'content', num_rows)
    #sleep(1)
    # Get only ids
...
def predict(filename):
    ...
    #sleep(0.3)
    df['hd2v_binary'] = df[['ground_truth', 'hd2v_recommendations']].progress_apply(
        lambda x: binarize_predictions(x.ground_truth, x.hd2v_recommendations), axis=1)
    print(df.hd2v_binary)
    sleep(0.3)
    df['lda_binary'] = df[['ground_truth', 'lda_recommendations']].progress_apply(
        lambda x: binarize_predictions(x.ground_truth, x.lda_recommendations), axis=1)
    sleep(0.3)
    df['bm25_binary'] = df[['ground_truth', 'bm25_recommendations']].progress_apply(
    ...
    df['hd2v_wv0dv1_binary'] = df[['ground_truth', 'hd2v_recommendations_wv0dv1']].progress_apply(
        lambda x: binarize_predictions(x.ground_truth, x.hd2v_recommendations_wv0dv1), axis=1)
    df['hybrid_binary'] = df[['ground_truth', 'hybrid_recommendations']].progress_apply(
        lambda x: binarize_predictions(x.ground_truth, x.hybrid_recommendations), axis=1)
    print("Binarized")
    df.to_pickle('/home/ashwath/Programs/MAGCS/Pickles/recommendations_mag50_bm25bothhd2v_hybrid_500_df_june10.pickle')
    print("Prediction done")
    calculate_metrics(df[['hd2v_recommendations', 'lda_recommendations', 'bm25_recommendations', 'hd2v_recommendations_wv0dv1', 'hybrid_recommendations',
                          'hd2v_binary', 'lda_binary', 'bm25_binary', 'hd2v_wv0dv1_binary', 'hybrid_binary', 'ground_truth']])
...
def calculate_hybrid_metrics(df):
    ...
    df.to_pickle('/home/ashwath/Programs/MAGCS/Pickles/paperwisemetrics_mag50_hybrid5050_df_may26.pickle')
    ...
    mean_series = df.mean()
    mean_series.to_csv('/home/ashwath/Programs/MAGCS/Evaluation/meanmetrics_mag50_may26_hybrid5050.tsv', sep='\t', index=True, header=False)
    print("C'est fini.")
...
def calculate_metrics(df):
    ...
    # 14 x 3 x 4 columns added for each
    for k in tqdm(klist):
        df['average_precision_hd2v_2vecs_{}'.format(k)] = df['hd2v_binary'].progress_apply(lambda x: average_precision(x, k))
        ...
        df['average_precision_hd2v_1vec_{}'.format(k)] = df['hd2v_wv0dv1_binary'].progress_apply(lambda x: average_precision(x, k))
        df['average_precision_hybrid_{}'.format(k)] = df['hybrid_binary'].progress_apply(lambda x: average_precision(x, k))
        #print(df)
        df['recall_hd2v_2vecs_{}'.format(k)] = df[['hd2v_binary', 'ground_truth']].progress_apply(
            lambda x: recall_at_k(x.hd2v_binary, x.ground_truth, k), axis=1)    
        #df['recall_lda_{}'.format(k)] = df[['lda_binary', 'ground_truth']].progress_apply(
        #    lambda x: recall_at_k(x.lda_binary, x.ground_truth, k), axis=1)
        df['recall_bm25_{}'.format(k)] = df[['bm25_binary', 'ground_truth']].progress_apply(
            lambda x: recall_at_k(x.bm25_binary, x.ground_truth, k), axis=1)
        df['recall_hd2v_1vec_{}'.format(k)] = df[['hd2v_wv0dv1_binary', 'ground_truth']].progress_apply(
            lambda x: recall_at_k(x.hd2v_wv0dv1_binary, x.ground_truth, k), axis=1)   
        df['recall_hybrid_{}'.format(k)] = df[['hybrid_binary', 'ground_truth']].apply(
            lambda x: recall_at_k(x.hybrid_binary, x.ground_truth, k), axis=1)
        df['reciprocal_rank_hd2v_2vecs_{}'.format(k)] = df['hd2v_binary'].progress_apply(lambda x: reciprocal_rank(x, k))
        ...
        df['reciprocal_rank_hd2v_1vec_{}'.format(k)] = df['hd2v_wv0dv1_binary'].progress_apply(lambda x: reciprocal_rank(x, k))
        df['reciprocal_rank_hybrid_{}'.format(k)] = df['hybrid_binary'].progress_apply(lambda x: reciprocal_rank(x, k))
        df['ndcg_hd2v_2vecs_{}'.format(k)] = df['hd2v_binary'].progress_apply(lambda x: ndcg(x, k))
        ...
        df['ndcg_hd2v_1vec_{}'.format(k)] = df['hd2v_wv0dv1_binary'].progress_apply(lambda x: ndcg(x, k))
        df['ndcg_hybrid_{}'.format(k)] = df['hybrid_binary'].progress_apply(lambda x: ndcg(x, k))
    df.to_pickle('/home/ashwath/Programs/MAGCS/Pickles/paperwisemetrics_mag50_4models_includinghybrid_df.pickle')
    print("METRICS CALCULATED, time to calculate the means")
    # Get the mean of all the index columns
    df = df.drop(['hd2v_recommendations', 'bm25_recommendations', 'hd2v_binary','bm25_binary', 'hd2v_recommendations_wv0dv1', 'hybrid_recommendations',
                  'ground_truth', 'hybrid_binary', 'lda_recommendations', 'lda_binary', 'hd2v_wv0dv1_binary'], axis=1)
    mean_series = df.mean()
    mean_series.to_csv('/home/ashwath/Programs/MAGCS/Evaluation/meanmetrics_mag50_bm25_2hd2v_hybridjune10.tsv', sep='\t', index=True, header=False)
    print("C'est fini.")


if __name__ == '__main__':
    filename = '/home/ashwath/Programs/MAGCS/mag50_testset_contexts_grouped.tsv'
    predict(filename)
