...
def remove_untraining_groundtruths(ground_truth_list):
    list_to_return = [paperid for paperid in ground_truth_list if paperid in hd2vmodel.docvecs.offset2doctag]
    return None if list_to_return==[] else list_to_return

def remove_duplicate_ground_truths(ground_truth_list):
    return list(set(ground_truth_list))
df['ground_truth'] = df['ground_truth'].apply(remove_duplicate_ground_truths)
df['ground_truth'] = df['ground_truth'].progress_apply(remove_untraining_groundtruths)
