...
query = 'select uuid,arxiv_id from bibitemarxividmap'
# 809498 entries returned
df = pd.read_sql_query(query, cconn)
df = df.set_index('uuid')

# Convert to a a series for fast dict-like lookup with uuid as the key and arxiv_id as the value.

series = df.mag_id
# Convert to a dict to get even faster lookup with uuid keys and mag_id values
uuid_mag_dict = series.to_dict()

with open('Pickles/uuid_arxivid_dict.pickle', 'wb') as picc:
    pickle.dump(uuid_arxiv_dict, picc)
