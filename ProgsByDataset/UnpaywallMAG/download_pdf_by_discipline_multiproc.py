...
basepath = '/datadrive/Files/Unpaywall'
...
def read_and_query(filepath):
    ...
    notfound_tsv = open('/datadrive/Files/Unpaywall/Fulltext/NotFound/pdf_not_found_{}_{}'.format(field, input_filename), 'a')
...
def create_concurrent_futures(filepath, field):
    """ Uses all the cores to do the parsing and inserting"""
    files = glob(os.path.join(filepath, field, '{}-part*.txt'.format(field)))
    #print(files, filepath, field)
    with concurrent.futures.ProcessPoolExecutor(max_workers=8) as executor:
        # chunk size =1
        executor.map(read_and_query, files)

def main():
    parser = argparse.ArgumentParser()
    parser.add_argument('field', help="Enter field of study (all small letters, no spaces between words)")
    args = parser.parse_args()
    filepath = '/datadrive/Files/PapersByFieldsOfStudy/'
    field = args.field.strip()
    create_concurrent_futures(filepath, field)
    print('done')
