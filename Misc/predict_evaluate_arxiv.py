...
def predict(filename):
    ...
    print("Binarized")
    df.to_pickle('/home/ashwath/Programs/ArxivCS/Pickles/recommendations_arxivmag_3models_500_df.pickle')
    df.to_csv('/home/ashwath/Programs/ArxivCS/Evaluation/recommendations_arxivmag_3models_500.tsv', sep='\t')
...
def calculate_metrics(df):
    ...
    klist.extend([20, 30, 40, 50])
    # 14 x 3 x 4 columns added for each
    for k in tqdm(klist):
        df['average_precision_hd2v_{}'.format(k)] = df['hd2v_binary'].apply(lambda x: average_precision(x, k))
        df['average_precision_lda_{}'.format(k)] = df['lda_binary'].apply(lambda x: average_precision(x, k))
        df['average_precision_bm25_{}'.format(k)] = df['bm25_binary'].apply(lambda x: average_precision(x, k))
        df['recall_hd2v_{}'.format(k)] = df[['hd2v_binary', 'ground_truth']].apply(
            lambda x: recall_at_k(x.hd2v_binary, x.ground_truth, k))    
        df['recall_lda_{}'.format(k)] = df[['lda_binary', 'ground_truth']].apply(
            lambda x: recall_at_k(x.lda_binary, x.ground_truth, k))
        df['recall_bm25_{}'.format(k)] = df[['bm25_binary', 'ground_truth']].apply(
            lambda x: recall_at_k(x.bm25_binary, x.ground_truth, k))
        df['reciprocal_rank_hd2v_{}'.format(k)] = df['hd2v_binary'].apply(lambda x: reciprocal_rank(x, k))
        df['reciprocal_rank_lda_{}'.format(k)] = df['lda_binary'].apply(lambda x: reciprocal_rank(x, k))
        df['reciprocal_rank_bm25_{}'.format(k)] = df['bm25_binary'].apply(lambda x: reciprocal_rank(x, k))
        df['ndcg_hd2v_{}'.format(k)] = df['hd2v_binary'].apply(lambda x: ndcg(x, k))
        df['ndcg_lda_{}'.format(k)] = df['lda_binary'].apply(lambda x: ndcg(x, k))
        df['ndcg_bm25_{}'.format(k)] = df['bm25_binary'].apply(lambda x: ndcg(x, k))
    df.to_csv('/home/ashwath/Programs/ArxivCS/Evaluation/paperwisemetrics_arxivmag_3models_500.tsv', sep='\t')
    df.to_pickle('/home/ashwath/Programs/ArxivCS/Pickles/paperwisemetrics_arxivmag_3models_df.pickle')
    print("METRICS CALCULATED, time to calculate the means")
    # Get the mean of all the index columns
    mean_series = df.mean()
    mean_series.to_csv('/home/ashwath/Programs/ArxivCS/Evaluation/', sep='\t', index=True, header=False)
    print("C'est fini.")
